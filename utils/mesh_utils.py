...
class GaussianExtractor(object):
    def __init__(self, gaussians, render, pipe, bg_color=None):
        """
        a class that extracts attributes from a scene presented by 3DGS/2DGS
    ...
    def reconstruction(self, viewpoint_stack):
        ...
        for i, viewpoint_cam in tqdm(enumerate(self.viewpoint_stack), desc="reconstruct radiance fields"):
            ...
            if valid.numel() > 0:
                ...
                all_depth_samples.append(flat[::stride].detach().cpu())
    ...
    def export_depth_stats(self, path, filename="depth_stats.json", extra=None):
        stats = dict(self.depth_stats)

        if extra is not None:
            stats["mesh_params"] = extra
        ...
        print("depth stats saved at {}".format(save_path))
    ...
    def extract_mesh_bounded(self, voxel_size=0.004, sdf_trunc=0.02, depth_trunc=3, mask_backgrond=True):
        ...
        for i, cam_o3d in tqdm(enumerate(to_cam_open3d(self.viewpoint_stack)), desc="TSDF integration progress"):
            ...
            if mask_backgrond:
                alpha_mask = getattr(self.viewpoint_stack[i], 'gt_alpha_mask', None) or \
                             getattr(self.viewpoint_stack[i], 'alpha_mask', None)
